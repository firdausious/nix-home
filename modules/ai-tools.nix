...
in {
  ...
  aiAliases = {
    ...
    "opencode-ai" = "cd $AI_WORKSPACE && opencode --model ollama/${
        aiConfig.model or "llama3.1:8b"
      }";
    "oc" = "opencode";
    "oc-ai" = "cd $AI_WORKSPACE && opencode --model ollama/${
        aiConfig.model or "llama3.1:8b"
      }";
    "oc-theme" = "opencode --theme catppuccin-mocha";
    "oc-transparent" =
      "OPENCODE_BACKGROUND_TRANSPARENCY=0.8 OPENCODE_BLUR=true opencode";
  ...
  aiSessionPath = [ "${homeDirectory}/${aiConfig.workspace}/bin" ];
}
